...
int signkey_generate(enum signkey_type keytype, int bits, const char* filename, int skip_exist)
{
	...
	buf_setpos(buf, 0);
       	...
       	/* Android (SElinux) does not allow to use hardlinks, so we write directly
	 * to the target file. As we don't have any concurrent access on this keyfile,
	 * there are no drawbacks for us...
	 */
